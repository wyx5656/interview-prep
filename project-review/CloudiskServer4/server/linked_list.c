...
void deleteNode2(ListNode **head, int peerfd)
{
    ...
    if (user->sockfd == peerfd) {
        ListNode *temp = *head;
        *head = (*head)->next;
        free(user); // 释放user_info_t结构体
        free(temp);
        printf("freed user and node for sockfd %d.\n", peerfd);
        return;
    }
    ...
    if (current->next != NULL) {
        ListNode *temp = current->next;
        user_info_t *user_to_free = (user_info_t*)temp->val;
        current->next = current->next->next;
        free(user_to_free); // 释放user_info_t结构体
        free(temp);
        printf("freed user and node 2 for sockfd %d.\n", peerfd);
        return;
    }
}
